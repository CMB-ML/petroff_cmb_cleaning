...
start_time_str = "2020-03-18-04-00-10"
...
FIELD = 0
MIN_MAX_FILENAME = SIM_DIR / "normalization.npz"
...
train_ds = (
    CmbDataset(NSIDE, planck_map_freqs, 0, N_TRAIN, SIM_DIR, False)
...
# Evaluate model
print("Train:")
print(model.evaluate(train_ds, verbose=1))
print("Test:")
...
train_ds_iter = iter(CmbDataset(NSIDE, planck_map_freqs, 0, N_TRAIN, SIM_DIR, False))
full_result = []
for _ in range(N_TRAIN):
    tiled_maps = np.tile(next(train_ds_iter)[0], [ERROR_CALC_NUM, 1, 1])
    result_comp = model.predict(tiled_maps, batch_size=BATCH_SIZE, verbose=1)
    results = result_comp[:, :, 0] + np.random.normal(scale=np.sqrt(np.exp(-result_comp[:, :, 1])))
    full_result.append(np.stack([np.mean(results, axis=0), -np.log(np.var(results, axis=0))], axis=1))
full_result = np.array(full_result)
# Un-normalize result, switching back to T_cmb (uK)
full_result[..., 0] = (full_result[..., 0] - 0.5) * y_max * 2
print('Training data:')
print('mean predicted error:', np.mean(np.sqrt(np.exp(-full_result[:, :, 1])) * y_max * 2))
#print('mean difference:', np.mean(np.abs((y_train - 0.5) * y_max * 2 - full_result[:, :, 0])))
#print('fraction within 1-sigma:', np.mean((np.abs((y_train - 0.5) * y_max * 2 - full_result[:, :, 0])) < np.sqrt(np.exp(-full_result[:, :, 1])) * y_max * 2))
np.savez_compressed(BASE_PATH / 'train.npz', full_result)



# Evaluate test data
test_ds_iter = iter(CmbDataset(NSIDE, planck_map_freqs, N_TRAIN, N_TEST, SIM_DIR, False))
full_result = []
for _ in range(N_TEST):
    tiled_maps = np.tile(next(test_ds_iter)[0], [ERROR_CALC_NUM, 1, 1])
    result_comp = model.predict(tiled_maps, batch_size=BATCH_SIZE, verbose=1)
    results = result_comp[:, :, 0] + np.random.normal(scale=np.sqrt(np.exp(-result_comp[:, :, 1])))
    full_result.append(np.stack([np.mean(results, axis=0), -np.log(np.var(results, axis=0))], axis=1))
full_result = np.array(full_result)
...
print('Test data:')
print('mean predicted error:', np.mean(np.sqrt(np.exp(-full_result[:, :, 1])) * y_max * 2))
#print('mean difference:', np.mean(np.abs((y_test - 0.5) * y_max * 2 - full_result[:, :, 0])))
#print('fraction within 1-sigma:', np.mean((np.abs((y_test - 0.5) * y_max * 2 - full_result[:, :, 0])) < np.sqrt(np.exp(-full_result[:, :, 1])) * y_max * 2))
np.savez_compressed(BASE_PATH / 'test.npz', full_result)'''



# Detailed evaluation of test example
#test_ds_iter = iter(CmbDataset(NSIDE, planck_map_freqs, 999, 1, SIM_DIR, False))
test_ds_iter = tfds.as_numpy(CmbDataset(NSIDE, planck_map_freqs, 999, 1, SIM_DIR, False))
tiled_maps = np.tile(next(test_ds_iter)[0], [ERROR_CALC_NUM, 1, 1])
result_comp = model.predict(tiled_maps, batch_size=BATCH_SIZE, verbose=1)
np.savez_compressed(BASE_PATH / 'test0999_raw.npz', result_comp)
results = result_comp[:, :, 0] + np.random.normal(scale=np.sqrt(np.exp(-result_comp[:, :, 1])))
full_result = np.stack([np.mean(results, axis=0), -np.log(np.var(results, axis=0))], axis=1)
# Un-normalize result, switching back to T_cmb (uK)
full_result[..., 0] = (full_result[..., 0] - 0.5) * y_max * 2
np.savez_compressed(BASE_PATH / 'test0999.npz', full_result)



# Detailed evaluation of training examples to check for bias in spectra
#train_ds_iter = iter(CmbDataset(NSIDE, planck_map_freqs, 0, N_TRAIN, SIM_DIR, False))
train_ds_iter = tfds.as_numpy(CmbDataset(NSIDE, planck_map_freqs, 0, N_TRAIN, SIM_DIR, False))
for i in range(100):
    tiled_maps = np.tile(next(train_ds_iter)[0], [ERROR_CALC_NUM, 1, 1])
    result_comp = model.predict(tiled_maps, batch_size=BATCH_SIZE, verbose=1)
    np.savez_compressed(BASE_PATH / f'train{i:04d}_raw.npz', result_comp)
...
for i, pmn in enumerate(planck_map_names):
    planck_maps.append(hp.reorder(hp.alm2map(hp.smoothalm(hp.map2alm(hp.read_map(PLANCK_MAP_DIR / pmn, verbose=False, field=FIELD), use_pixel_weights=True), beam_window=hp.gauss_beam(np.deg2rad(13.1 / 60), 3 * NSIDE) / hp.gauss_beam(instr_beams[i], 3 * NSIDE)), NSIDE, pixwin=True), r2n=True))
...
np.savez_compressed(BASE_PATH / 'planck_raw.npz', planck_result)
results = planck_result[:, :, 0] + np.random.normal(scale=np.sqrt(np.exp(-planck_result[:, :, 1])))
planck_result = np.stack([np.mean(results, axis=0), -np.log(np.var(results, axis=0))], axis=1)

# Un-normalize result, switching back to T_cmb (uK)
planck_result[..., 0] = (planck_result[..., 0] - 0.5) * y_max * 2

np.savez_compressed(BASE_PATH / 'planck.npz', planck_result)
